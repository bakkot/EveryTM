...
#define TICKS_PER_PASS 1000
#define MACHINES_PER_FILE 1000

// #define USE_THREADS // on my machine, this is significantly... slower. Like, 30-50% slower.
...
int main() {
  ...
  if(thread_count < 2) {
    cerr << "Compiled with multithreading, but hardware_concurrency() reports no multithreading support." << endl;
    thread_count = 1;
  }
  cout << thread_count << " threads being deployed." << endl; // todo
#endif
  ...
  for(int i=0; i<10; ++i) { // TODO while(true)
	...
	if(!getline(in_file, line)) {
	  cerr << file_name_buf << " contains too few machines or unreadable; aborting!" << endl;
	  exit(1);
	}
    ...
    for(int i=0; i<thread_count; i++) {
      workers.push_back(thread(RunBoxes, ref(box)));
    }
    // join
